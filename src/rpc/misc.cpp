...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total reibit balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zRITsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zRIT denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zRIT denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zRIT denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zRIT denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zRIT denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zRIT denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zRIT denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zRIT denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zRIT denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in reibit/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in reibit/kb\n"
    ...
    UniValue zritObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zritObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zritObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zRITsupply", zritObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"reibitaddress\"\n"
            "\nReturn information about the given reibit address.\n"

            "\nArguments:\n"
            "1. \"reibitaddress\"     (string, required) The reibit address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"reibitaddress\", (string) The reibit address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: ReiBit address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are reibit addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) reibit address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"reibitaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"reibitaddress\"  (string, required) The reibit address to use for the signature.\n"
