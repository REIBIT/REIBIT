...
// The zRIT block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zRIT is a hash of the serial
...
{
    //Create an output returning the zRIT that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZRITOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zRIT output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zRIT", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZRITOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zRIT output", __func__);
...
{
    CzRITTracker* zritTracker = pwallet->zritTracker.get();
    CMintMeta meta;
    if (!zritTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zritTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a RIT stake is the outpoint
