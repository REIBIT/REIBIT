...
#include "ui_zritcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zritTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zritTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zRIT.";
            else if (!mint.isSeedCorrect)
                strReason = "The zRIT seed used to mint this zRIT is not the same as currently hold in the wallet";
            else
