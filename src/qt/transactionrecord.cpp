...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZRITModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zritTracker->HasMintTx(hash))) {
            //zRIT stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZRIT;
...
bool IsZRITType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZRIT:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZRIT || type == TransactionRecord::MNReward) {
