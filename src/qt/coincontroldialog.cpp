...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::RIT;
    ...
    QString toolTip4 = tr("Can vary +/- %1 urit per input.").arg(dFeeVary);
    ...
    for (PAIRTYPE(QString, vector<COutput>) coins : mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show ReiBit address. In tree mode, address is not shown again for direct wallet address outputs
