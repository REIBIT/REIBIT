...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zritspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 RIT at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zRIT send
enum ZerocoinSpendStatus {
    ZRIT_SPEND_OKAY = 0,                            // No error
    ZRIT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZRIT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZRIT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZRIT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZRIT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZRIT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZRIT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZRIT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZRIT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZRIT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZRIT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZRIT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZRIT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZRIT_SPENT_USED_ZRIT = 14,                      // Coin has already been spend
    ZRIT_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZRIT_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZRITOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzritAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzRITTracker> zritTracker;
    ...
    void setZWallet(CzRITWallet* zwallet)
    {
        zwalletMain = zwallet;
        zritTracker = std::unique_ptr<CzRITTracker>(new CzRITTracker(strWalletFile));
    }

    CzRITWallet* getZWallet() { return zwalletMain; }
    ...
    /** zRIT reset */
    boost::signals2::signal<void()> NotifyzRITReset;
