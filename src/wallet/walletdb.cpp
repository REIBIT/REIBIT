...
bool CWalletDB::WriteDeterministicMint(const CDeterministicMint& dMint)
{
    ...
    return Write(make_pair(string("dzrit"), hash), dMint, true);
}

bool CWalletDB::ReadDeterministicMint(const uint256& hashPubcoin, CDeterministicMint& dMint)
{
    return Read(make_pair(string("dzrit"), hashPubcoin), dMint);
}

bool CWalletDB::EraseDeterministicMint(const uint256& hashPubcoin)
{
    return Erase(make_pair(string("dzrit"), hashPubcoin));
}
...
bool CWalletDB::ArchiveDeterministicOrphan(const CDeterministicMint& dMint)
{
    ...
    if (!Erase(make_pair(string("dzrit"), dMint.GetPubcoinHash())))
...
bool CWalletDB::WriteZRITSeed(const uint256& hashSeed, const vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZRITSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZRITSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZRITSeed_deprecated()
{
    return Erase(string("dzs"));
}

bool CWalletDB::ReadZRITSeed(const uint256& hashSeed, vector<unsigned char>& seed)
{
    return Read(make_pair(string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZRITSeed_deprecated(uint256& seed)
{
    return Read(string("dzs"), seed);
}

bool CWalletDB::WriteZRITCount(const uint32_t& nCount)
{
    return Write(string("dzc"), nCount);
}

bool CWalletDB::ReadZRITCount(uint32_t& nCount)
{
...
std::list<CDeterministicMint> CWalletDB::ListDeterministicMints()
{
    ...
    for (;;)
    {
        ...
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        if (fFlags == DB_SET_RANGE)
            ssKey << make_pair(string("dzrit"), uint256(0));
        CDataStream ssValue(SER_DISK, CLIENT_VERSION);
        int ret = ReadAtCursor(pcursor, ssKey, ssValue, fFlags);
        fFlags = DB_NEXT;
        if (ret == DB_NOTFOUND)
