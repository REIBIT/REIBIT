...
public:
    ...
    bool WriteZRITSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZRITSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZRITSeed_deprecated(uint256& seed);
    bool EraseZRITSeed();
    bool EraseZRITSeed_deprecated();

    bool WriteZRITCount(const uint32_t& nCount);
    bool ReadZRITCount(uint32_t& nCount);
