...
CzRITWallet::CzRITWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zrit seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRITSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRITSeed_deprecated()) {
                    LogPrintf("%s: Updated zRIT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zrit seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zrit wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRITWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRITCount(nCountLastUsed);
    else if (!walletdb.ReadZRITCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRITWallet::Lock()
{
    seedMaster = 0;
}

void CzRITWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRITWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRIT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRITWallet::LoadMintPoolFromDB()
{
...
void CzRITWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRITWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRITWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zritTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRITWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRIT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zritTracker which also adds to database
    pwalletMain->zritTracker->Add(dMint, true);
...
void CzRITWallet::SeedToZRIT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRITWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRITWallet::GenerateDeterministicZRIT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRITWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRITWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzRITWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
