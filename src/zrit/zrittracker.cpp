...
CzRITTracker::CzRITTracker(std::string strWalletFile)
{
...
CzRITTracker::~CzRITTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzRITTracker::Init()
{
...
bool CzRITTracker::Archive(CMintMeta& meta)
{
...
bool CzRITTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzRITTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzRITTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzRITTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzRITTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzRITTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzRITTracker::GetSerialHashes()
{
...
CAmount CzRITTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzRITTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzRITTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzRITTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzRITTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzRITTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzRITTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzRITTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzRITTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzRITTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzRITTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzRITWallet* zRITWallet)
{
    bool iszRITWalletInitialized = (NULL != zRITWallet);
    CMintMeta meta;
    ...
    if (! iszRITWalletInitialized)
        zRITWallet = new CzRITWallet(strWalletFile);
    meta.isSeedCorrect = zRITWallet->CheckSeed(dMint);
    if (! iszRITWalletInitialized)
        delete zRITWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzRITTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzRITTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzRITTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzRITTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzRITTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzRITTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzRITWallet* zRITWallet = new CzRITWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zRITWallet);
        }
        delete zRITWallet;
        LogPrint("zero", "%s: added %d dzrit from DB\n", __func__, listDeterministicDB.size());
    }
