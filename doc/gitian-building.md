...
*Setup instructions for a gitian build of ReiBit using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ReiBit
...
- [Building ReiBit](#building-reibit)
...
Clone the git repositories for reibit and gitian and then checkout the reibit version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/reibit-crypto/reibit.git
cd reibit
...
Follow the instructions in [doc/release-process.md](release-process.md) in the reibit repository
...
To build ReiBit (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the reibit repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/reibit/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/reibit-crypto/reibit
...
For example:
```bash
URL=https://github.com/crowning-/reibit.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit reibit=${COMMIT} --url reibit=${URL} ../reibit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit reibit=${COMMIT} --url reibit=${URL} ../reibit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit reibit=${COMMIT} --url reibit=${URL} ../reibit/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/reibit-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/reibit-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/reibit-build.assert
```
...
[reibit/gitian.sigs](https://github.com/reibit-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
